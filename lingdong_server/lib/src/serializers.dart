//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//

// ignore_for_file: unused_import

import 'package:one_of_serializer/any_of_serializer.dart';
import 'package:one_of_serializer/one_of_serializer.dart';
import 'package:built_collection/built_collection.dart';
import 'package:built_value/json_object.dart';
import 'package:built_value/serializer.dart';
import 'package:built_value/standard_json_plugin.dart';
import 'package:built_value/iso_8601_date_time_serializer.dart';
import 'package:lingdong_server/src/date_serializer.dart';
import 'package:lingdong_server/src/model/date.dart';

import 'package:lingdong_server/src/model/beautification_record_create.dart';
import 'package:lingdong_server/src/model/beautification_record_response.dart';
import 'package:lingdong_server/src/model/behavior_style_create.dart';
import 'package:lingdong_server/src/model/behavior_style_response.dart';
import 'package:lingdong_server/src/model/body_delete_media_api_media_delete_media_post.dart';
import 'package:lingdong_server/src/model/body_upload_media_api_media_upload_media_post.dart';
import 'package:lingdong_server/src/model/breed_detail.dart';
import 'package:lingdong_server/src/model/change_password_request.dart';
import 'package:lingdong_server/src/model/community_create.dart';
import 'package:lingdong_server/src/model/community_detail_response.dart';
import 'package:lingdong_server/src/model/community_list_response.dart';
import 'package:lingdong_server/src/model/community_member_list_response.dart';
import 'package:lingdong_server/src/model/community_member_response.dart';
import 'package:lingdong_server/src/model/community_response.dart';
import 'package:lingdong_server/src/model/consultation_create.dart';
import 'package:lingdong_server/src/model/consultation_response.dart';
import 'package:lingdong_server/src/model/deworming_record_create.dart';
import 'package:lingdong_server/src/model/deworming_record_response.dart';
import 'package:lingdong_server/src/model/dog_age_stage_base.dart';
import 'package:lingdong_server/src/model/dog_age_stage_create.dart';
import 'package:lingdong_server/src/model/dog_age_stage_out.dart';
import 'package:lingdong_server/src/model/dog_age_stage_update.dart';
import 'package:lingdong_server/src/model/dog_behavior_guide_base.dart';
import 'package:lingdong_server/src/model/dog_behavior_guide_create.dart';
import 'package:lingdong_server/src/model/dog_behavior_guide_out.dart';
import 'package:lingdong_server/src/model/dog_behavior_guide_update.dart';
import 'package:lingdong_server/src/model/dog_breed_create.dart';
import 'package:lingdong_server/src/model/dog_breed_delete.dart';
import 'package:lingdong_server/src/model/dog_breed_list_response.dart';
import 'package:lingdong_server/src/model/dog_breed_out.dart';
import 'package:lingdong_server/src/model/dog_breed_response.dart';
import 'package:lingdong_server/src/model/dog_breed_update.dart';
import 'package:lingdong_server/src/model/dog_care_guide_base.dart';
import 'package:lingdong_server/src/model/dog_care_guide_create.dart';
import 'package:lingdong_server/src/model/dog_care_guide_out.dart';
import 'package:lingdong_server/src/model/dog_care_guide_update.dart';
import 'package:lingdong_server/src/model/dog_training_guide_create.dart';
import 'package:lingdong_server/src/model/dog_training_guide_out.dart';
import 'package:lingdong_server/src/model/dog_training_guide_update.dart';
import 'package:lingdong_server/src/model/examination_record_create.dart';
import 'package:lingdong_server/src/model/examination_record_response.dart';
import 'package:lingdong_server/src/model/generic_response_beautification_record_response.dart';
import 'package:lingdong_server/src/model/generic_response_behavior_style_response.dart';
import 'package:lingdong_server/src/model/generic_response_bool.dart';
import 'package:lingdong_server/src/model/generic_response_community_detail_response.dart';
import 'package:lingdong_server/src/model/generic_response_community_list_response.dart';
import 'package:lingdong_server/src/model/generic_response_community_member_list_response.dart';
import 'package:lingdong_server/src/model/generic_response_community_response.dart';
import 'package:lingdong_server/src/model/generic_response_consultation_response.dart';
import 'package:lingdong_server/src/model/generic_response_deworming_record_response.dart';
import 'package:lingdong_server/src/model/generic_response_dict.dart';
import 'package:lingdong_server/src/model/generic_response_dog_age_stage_out.dart';
import 'package:lingdong_server/src/model/generic_response_dog_behavior_guide_out.dart';
import 'package:lingdong_server/src/model/generic_response_dog_breed_list_response.dart';
import 'package:lingdong_server/src/model/generic_response_dog_breed_out.dart';
import 'package:lingdong_server/src/model/generic_response_dog_care_guide_out.dart';
import 'package:lingdong_server/src/model/generic_response_empty_response.dart';
import 'package:lingdong_server/src/model/generic_response_examination_record_response.dart';
import 'package:lingdong_server/src/model/generic_response_int.dart';
import 'package:lingdong_server/src/model/generic_response_list.dart';
import 'package:lingdong_server/src/model/generic_response_list_behavior_style_response.dart';
import 'package:lingdong_server/src/model/generic_response_list_dog_age_stage_out.dart';
import 'package:lingdong_server/src/model/generic_response_list_personality_response.dart';
import 'package:lingdong_server/src/model/generic_response_list_place_response.dart';
import 'package:lingdong_server/src/model/generic_response_list_task_type_response.dart';
import 'package:lingdong_server/src/model/generic_response_login_response.dart';
import 'package:lingdong_server/src/model/generic_response_media_out.dart';
import 'package:lingdong_server/src/model/generic_response_other_health_record_response.dart';
import 'package:lingdong_server/src/model/generic_response_paged_response_dog_behavior_guide_out.dart';
import 'package:lingdong_server/src/model/generic_response_paged_response_dog_care_guide_out.dart';
import 'package:lingdong_server/src/model/generic_response_pagination_response_beautification_record_response.dart';
import 'package:lingdong_server/src/model/generic_response_pagination_response_consultation_response.dart';
import 'package:lingdong_server/src/model/generic_response_pagination_response_deworming_record_response.dart';
import 'package:lingdong_server/src/model/generic_response_pagination_response_examination_record_response.dart';
import 'package:lingdong_server/src/model/generic_response_pagination_response_other_health_record_response.dart';
import 'package:lingdong_server/src/model/generic_response_pagination_response_vaccination_record_response.dart';
import 'package:lingdong_server/src/model/generic_response_pagination_response_weight_record_response.dart';
import 'package:lingdong_server/src/model/generic_response_parent_comment.dart';
import 'package:lingdong_server/src/model/generic_response_personality_response.dart';
import 'package:lingdong_server/src/model/generic_response_pet_list_response.dart';
import 'package:lingdong_server/src/model/generic_response_pet_response.dart';
import 'package:lingdong_server/src/model/generic_response_phone_code_login_response.dart';
import 'package:lingdong_server/src/model/generic_response_phone_login_response.dart';
import 'package:lingdong_server/src/model/generic_response_place_response.dart';
import 'package:lingdong_server/src/model/generic_response_post_list_response.dart';
import 'package:lingdong_server/src/model/generic_response_post_response.dart';
import 'package:lingdong_server/src/model/generic_response_refresh_token_response.dart';
import 'package:lingdong_server/src/model/generic_response_reply.dart';
import 'package:lingdong_server/src/model/generic_response_send_code_response.dart';
import 'package:lingdong_server/src/model/generic_response_tag_list_response.dart';
import 'package:lingdong_server/src/model/generic_response_tag_response.dart';
import 'package:lingdong_server/src/model/generic_response_task_type_response.dart';
import 'package:lingdong_server/src/model/generic_response_user_info_response.dart';
import 'package:lingdong_server/src/model/generic_response_vaccination_record_response.dart';
import 'package:lingdong_server/src/model/generic_response_weight_record_response.dart';
import 'package:lingdong_server/src/model/http_validation_error.dart';
import 'package:lingdong_server/src/model/location_inner.dart';
import 'package:lingdong_server/src/model/login_request.dart';
import 'package:lingdong_server/src/model/login_response.dart';
import 'package:lingdong_server/src/model/media_create.dart';
import 'package:lingdong_server/src/model/media_out.dart';
import 'package:lingdong_server/src/model/media_response.dart';
import 'package:lingdong_server/src/model/media_type.dart';
import 'package:lingdong_server/src/model/medication.dart';
import 'package:lingdong_server/src/model/membership_approval.dart';
import 'package:lingdong_server/src/model/other_health_record_create.dart';
import 'package:lingdong_server/src/model/other_health_record_response.dart';
import 'package:lingdong_server/src/model/paged_response_dog_behavior_guide_out.dart';
import 'package:lingdong_server/src/model/paged_response_dog_care_guide_out.dart';
import 'package:lingdong_server/src/model/pagination_params.dart';
import 'package:lingdong_server/src/model/pagination_response_beautification_record_response.dart';
import 'package:lingdong_server/src/model/pagination_response_consultation_response.dart';
import 'package:lingdong_server/src/model/pagination_response_deworming_record_response.dart';
import 'package:lingdong_server/src/model/pagination_response_examination_record_response.dart';
import 'package:lingdong_server/src/model/pagination_response_other_health_record_response.dart';
import 'package:lingdong_server/src/model/pagination_response_vaccination_record_response.dart';
import 'package:lingdong_server/src/model/pagination_response_weight_record_response.dart';
import 'package:lingdong_server/src/model/parent_comment.dart';
import 'package:lingdong_server/src/model/parent_comment_create.dart';
import 'package:lingdong_server/src/model/personality_create.dart';
import 'package:lingdong_server/src/model/personality_response.dart';
import 'package:lingdong_server/src/model/pet_create.dart';
import 'package:lingdong_server/src/model/pet_list_response.dart';
import 'package:lingdong_server/src/model/pet_response.dart';
import 'package:lingdong_server/src/model/phone_code_login_request.dart';
import 'package:lingdong_server/src/model/phone_code_login_response.dart';
import 'package:lingdong_server/src/model/phone_login_request.dart';
import 'package:lingdong_server/src/model/phone_login_response.dart';
import 'package:lingdong_server/src/model/place_create.dart';
import 'package:lingdong_server/src/model/place_response.dart';
import 'package:lingdong_server/src/model/post_create.dart';
import 'package:lingdong_server/src/model/post_list_response.dart';
import 'package:lingdong_server/src/model/post_response.dart';
import 'package:lingdong_server/src/model/refresh_token_response.dart';
import 'package:lingdong_server/src/model/related_type_enum.dart';
import 'package:lingdong_server/src/model/reply.dart';
import 'package:lingdong_server/src/model/reply_create.dart';
import 'package:lingdong_server/src/model/role_update.dart';
import 'package:lingdong_server/src/model/send_code_request.dart';
import 'package:lingdong_server/src/model/send_code_response.dart';
import 'package:lingdong_server/src/model/set_password_by_code_request.dart';
import 'package:lingdong_server/src/model/tag.dart';
import 'package:lingdong_server/src/model/tag_create.dart';
import 'package:lingdong_server/src/model/tag_list_response.dart';
import 'package:lingdong_server/src/model/tag_response.dart';
import 'package:lingdong_server/src/model/tag_update.dart';
import 'package:lingdong_server/src/model/task_type_create.dart';
import 'package:lingdong_server/src/model/task_type_response.dart';
import 'package:lingdong_server/src/model/user_info_response.dart';
import 'package:lingdong_server/src/model/vaccination_record_create.dart';
import 'package:lingdong_server/src/model/vaccination_record_response.dart';
import 'package:lingdong_server/src/model/validation_error.dart';
import 'package:lingdong_server/src/model/weight_record_create.dart';
import 'package:lingdong_server/src/model/weight_record_response.dart';

part 'serializers.g.dart';

@SerializersFor([
  BeautificationRecordCreate,
  BeautificationRecordResponse,
  BehaviorStyleCreate,
  BehaviorStyleResponse,
  BodyDeleteMediaApiMediaDeleteMediaPost,
  BodyUploadMediaApiMediaUploadMediaPost,
  BreedDetail,
  ChangePasswordRequest,
  CommunityCreate,
  CommunityDetailResponse,
  CommunityListResponse,
  CommunityMemberListResponse,
  CommunityMemberResponse,
  CommunityResponse,
  ConsultationCreate,
  ConsultationResponse,
  DewormingRecordCreate,
  DewormingRecordResponse,
  DogAgeStageBase,
  DogAgeStageCreate,
  DogAgeStageOut,
  DogAgeStageUpdate,
  DogBehaviorGuideBase,
  DogBehaviorGuideCreate,
  DogBehaviorGuideOut,
  DogBehaviorGuideUpdate,
  DogBreedCreate,
  DogBreedDelete,
  DogBreedListResponse,
  DogBreedOut,
  DogBreedResponse,
  DogBreedUpdate,
  DogCareGuideBase,
  DogCareGuideCreate,
  DogCareGuideOut,
  DogCareGuideUpdate,
  DogTrainingGuideCreate,
  DogTrainingGuideOut,
  DogTrainingGuideUpdate,
  ExaminationRecordCreate,
  ExaminationRecordResponse,
  GenericResponseBeautificationRecordResponse,
  GenericResponseBehaviorStyleResponse,
  GenericResponseBool,
  GenericResponseCommunityDetailResponse,
  GenericResponseCommunityListResponse,
  GenericResponseCommunityMemberListResponse,
  GenericResponseCommunityResponse,
  GenericResponseConsultationResponse,
  GenericResponseDewormingRecordResponse,
  GenericResponseDict,
  GenericResponseDogAgeStageOut,
  GenericResponseDogBehaviorGuideOut,
  GenericResponseDogBreedListResponse,
  GenericResponseDogBreedOut,
  GenericResponseDogCareGuideOut,
  GenericResponseEmptyResponse,
  GenericResponseExaminationRecordResponse,
  GenericResponseInt,
  GenericResponseList,
  GenericResponseListBehaviorStyleResponse,
  GenericResponseListDogAgeStageOut,
  GenericResponseListPersonalityResponse,
  GenericResponseListPlaceResponse,
  GenericResponseListTaskTypeResponse,
  GenericResponseLoginResponse,
  GenericResponseMediaOut,
  GenericResponseOtherHealthRecordResponse,
  GenericResponsePagedResponseDogBehaviorGuideOut,
  GenericResponsePagedResponseDogCareGuideOut,
  GenericResponsePaginationResponseBeautificationRecordResponse,
  GenericResponsePaginationResponseConsultationResponse,
  GenericResponsePaginationResponseDewormingRecordResponse,
  GenericResponsePaginationResponseExaminationRecordResponse,
  GenericResponsePaginationResponseOtherHealthRecordResponse,
  GenericResponsePaginationResponseVaccinationRecordResponse,
  GenericResponsePaginationResponseWeightRecordResponse,
  GenericResponseParentComment,
  GenericResponsePersonalityResponse,
  GenericResponsePetListResponse,
  GenericResponsePetResponse,
  GenericResponsePhoneCodeLoginResponse,
  GenericResponsePhoneLoginResponse,
  GenericResponsePlaceResponse,
  GenericResponsePostListResponse,
  GenericResponsePostResponse,
  GenericResponseRefreshTokenResponse,
  GenericResponseReply,
  GenericResponseSendCodeResponse,
  GenericResponseTagListResponse,
  GenericResponseTagResponse,
  GenericResponseTaskTypeResponse,
  GenericResponseUserInfoResponse,
  GenericResponseVaccinationRecordResponse,
  GenericResponseWeightRecordResponse,
  HttpValidationError,
  LocationInner,
  LoginRequest,
  LoginResponse,
  MediaCreate,
  MediaOut,
  MediaResponse,
  MediaType,
  Medication,
  MembershipApproval,
  OtherHealthRecordCreate,
  OtherHealthRecordResponse,
  PagedResponseDogBehaviorGuideOut,
  PagedResponseDogCareGuideOut,
  PaginationParams,
  PaginationResponseBeautificationRecordResponse,
  PaginationResponseConsultationResponse,
  PaginationResponseDewormingRecordResponse,
  PaginationResponseExaminationRecordResponse,
  PaginationResponseOtherHealthRecordResponse,
  PaginationResponseVaccinationRecordResponse,
  PaginationResponseWeightRecordResponse,
  ParentComment,
  ParentCommentCreate,
  PersonalityCreate,
  PersonalityResponse,
  PetCreate,
  PetListResponse,
  PetResponse,
  PhoneCodeLoginRequest,
  PhoneCodeLoginResponse,
  PhoneLoginRequest,
  PhoneLoginResponse,
  PlaceCreate,
  PlaceResponse,
  PostCreate,
  PostListResponse,
  PostResponse,
  RefreshTokenResponse,
  RelatedTypeEnum,
  Reply,
  ReplyCreate,
  RoleUpdate,
  SendCodeRequest,
  SendCodeResponse,
  SetPasswordByCodeRequest,
  Tag,
  TagCreate,
  TagListResponse,
  TagResponse,
  TagUpdate,
  TaskTypeCreate,
  TaskTypeResponse,
  UserInfoResponse,
  VaccinationRecordCreate,
  VaccinationRecordResponse,
  ValidationError,
  WeightRecordCreate,
  WeightRecordResponse,
])
Serializers serializers = (_$serializers.toBuilder()
      ..addBuilderFactory(
        const FullType(BuiltList, [FullType(DogTrainingGuideOut)]),
        () => ListBuilder<DogTrainingGuideOut>(),
      )
      ..addBuilderFactory(
        const FullType(BuiltMap, [FullType(String), FullType(JsonObject)]),
        () => MapBuilder<String, JsonObject>(),
      )
      ..add(const OneOfSerializer())
      ..add(const AnyOfSerializer())
      ..add(const DateSerializer())
      ..add(Iso8601DateTimeSerializer())
    ).build();

Serializers standardSerializers =
    (serializers.toBuilder()..addPlugin(StandardJsonPlugin())).build();
